...
<head>
       ...
       <link here href="https://cdn.jsdelivr.net/npm/bootstrap@5.2.0-beta1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-0evHe/X+R7YkIZDRvuzKMRqM+OrBnVFBL6DOitfPri4tjfHxaWutUpFmBp4vmVor">
</head>
  ...
  <div class = "row mb-3">
        ...
        <div class = " col-sm-4">
            <select name = "pais" id = "pais" class = "form-select">
               ...
               <option value = "CL" ${param.pais.equals("ES")? "selected": ""}> España </option>
               <option value = "AR" ${param.pais.equals("AR")? "selected": ""}> Argentina </option>
               <option value = "PE" ${param.pais.equals("PR")? "selected": ""}> Peru </option>
 ...
 <div class = "row mb-3">
          <div class = "col-sm-4">
             ...
             <select name = "lenguajes" id = "lenguajes" multiple class = "form-select">
                <option value = "java" ${paramValues.lenguajes.stream().anyMath(v->v.equals("java")).get()?"selected":""}>Java SE </option>
                <option value = "jakartaee" ${paramValues.lenguajes.stream().anyMath(v->v.equals("jakartaee")).get()?"selected":""}>Jakarta EE9 </option>
                <option value = "spring" ${paramValues.lenguajes.stream().anyMath(v->v.equals("spring")).get()?"selected":""}> Spring Boot </option>
                <option value = "js" ${paramValues.lenguajes.stream().anyMath(v->v.equals("js")).get()?"selected":""}> JavaScrip </option>
                <option value = "angular" ${paramValues.lenguajes.stream().anyMath(v->v.equals("angular")).get()?"selected":""}> Angular </option>
                <option value = "react" ${paramValues.lenguajes.stream().anyMath(v->v.equals("react")).get()?"selected":""}> React </option>
             </select>
         </div>
       <%
   ...
   <div class = "row mb-3">
               <div class = "form-check col-sm-2">
                 <label class = "col-form-label col-sm-2">Roles</label>
                  ...
                  ${paramValues.roles.stream().anyMatch(v->v.equals("ROLE_ADMIN")).get()?"checked":""} class = "form-check-input">
                  <label class = "form-check-label">Administrador</label>
              </div>

              <div class = "form-check col-sm-2">
                <input type = "checkbox" name="roles" value="ROLES_USER" class = "form-check-input"
                ${paramValues.roles.stream().anyMatch(v->v.equals("ROLES_USER").get()?"checked":""}>
                <label class = "form.check-label">Usuario</label>
            </div>

             <div class = "form-check col-sm-2">
                  <input type = "checkbox" name="roles" value="ROLES_MODERATOR" class="form-check-input" ${paramValues.roles.stream().anyMatch(v->v.equals("ROLE_MODERATOR")).get()?"checked":""}>
